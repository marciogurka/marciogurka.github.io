		...
		fetch(link, { headers: { "Content-Type": "application/json; charset=utf-8" }})
		    ...
		    .then(response => {
		    	...
		    	if (response.items.length) {
		    		...
		    		let postList = $(".schedule-list");
		    		postList.empty();
		    		items.forEach((item) => {
		    			let li = $("<li></li>").addClass("schedule-item");
				    	let title = $("<a></a>").addClass("schedule-title");
				    	let pubDate = $("<p></p>").addClass("schedule-date");
			    		let date = new Date(item.pubDate);
			    		title.text(item.title);
			    		pubDate.text(date.toLocaleDateString());
			    		title.attr(item.link)
			    		li.append(title);
			    		li.append(pubDate);
			    		postList.append(li);
			    	})
		    	} else {
		    		let postListText = $(".schedule-list").children();
		    		postListText.text("No posts yet");
		    	}
		    })
		    .catch(err => {
		    	let postListText = $(".schedule-list").children();
		        postListText.text("Error getting the posts! Please reload the page or check the Medium Profile link");
		    });
	});
